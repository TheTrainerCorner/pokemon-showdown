...
export class RandomTeams {
	...
	constructor(format: Format | string, prng: PRNG | PRNGSeed | null) {
		...
		this.moveEnforcementCheckers = {
			...
			Cosmic: (movePool, move, abilities, types, counter) => !counter.get('Cosmic'),
	...
	getAbility(
		...
		if (species.id === 'venusaur' && role === 'Leftovers') return 'Overgrow';

		if (species.id === 'charizard' && role === 'Heavy Duty Boots') return 'Blaze';
		...
		if (species.id === 'raticatealola' && role === 'Assault Vest') return 'Thick Fat';
		if (species.id === 'raticatealola' && role === 'Sitrus Berry') return 'Cheek Pouch';
		...
		if (species.id === 'dugtrioalola' && role === 'Choice Band') return 'Emergency Exit';
		if (species.id === 'dugtrioalola' && role === 'Clear Amulet') return 'Tangling Hair';
		if (species.id === 'dugtrioalola' && role === 'Focus Sash') return 'Tangling Hair';
		...
		if (species.id === 'persianalola' && role === 'Heavy Duty Boots') return 'Prankster';
		if (species.id === 'persianalola' && role === 'Life Orb') return 'Fur Coat';
		if (species.id === 'persianalola' && role === 'Leftovers') return 'Fur Coat';
		...
		if (species.id === 'mew' && role === 'Leftovers') return 'Illusion';
		if (species.id === 'mew' && role === 'Armor Plate') return 'Illusion';
		if (species.id === 'mew' && role === 'Weakness Policy') return 'Synchronize';
		...
		// 	if (species.id === 'zebstrika') return 'Sap Sipper';
	...
	getPriorityItem(
		...
		switch (role) {
			...
			case "Armor Plate": return "Armor Plate";
			case "Weakness Policy": return "Weakness Policy";
			case "Choice Scarf": return "Choice Scarf";
			case "Heavy Duty Boots": return "Heavy-Duty Boots";
			...
			case 'Clear Amulet': return 'Clear Amulet';
		}
		...
		// 	if (species.id === 'necrozmaduskmane') return 'Weakness Policy';
		// }
