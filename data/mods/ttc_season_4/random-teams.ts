...
export class RandomTTCTeams extends RandomGen8Teams {
	...
	constructor(format: Format | string, prng: PRNG | PRNGSeed | null) {
		...
		this.moveEnforcementCheckers = {
			...
			Steel: (movepool, moves, abilities, types, counter) => !counter.get('Steel'),
			Water: (movepool, moves, abilities, types, counter) => !counter.get('Water'),
		};
	}

	queryMoves(moves: Set<string> | null, types: string[], abilities: Set<string>, movepool: string[]): MoveCounter {
	...
	shouldCullMove(move: Move, types: Set<string>, moves: Set<string>, abilities: Set<string>, counter: MoveCounter, movepool: string[], teamDetails: RandomTeamsTypes.TeamDetails, species: Species, isLead: boolean, isDoubles: boolean): { cull: boolean; isSetup?: boolean; } {
		if (isDoubles && species.baseStats.def >= 140 && movepool.includes('bodypress')) return { cull: true };
	...
	randomSet(
		...
		const movePool = (randMoves || Object.keys(this.dex.species.getFullLearnset(species.id)!)).slice();
	...
	randomTeam() {
		...
		const pokemonList = [];
		for (const poke of this.dex.species.all()) {
			pokemonList.push(poke.id);
		}
		const pokemonPool = this.getPokemonPool(type, pokemon, isMonotype, pokemonList);
