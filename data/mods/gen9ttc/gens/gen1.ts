...
export const Gen1 = (dex: ModdedDex) => {
	...
	let learnsetRemove = (pokemon: string, moves: string[]) => {
		for(let move of moves) {
			dex.modData('Learnsets', pokemon.toLowerCase()).learnset[move.toLowerCase()] = [];
		}
	}

	let learnsetAdd = (pokemon: string, moves: [string, number][]) => {
		for(let move of moves) {
			// Example: dex.modData('Learnset', 'furret').learset['playrough'] = ['9M'];
			dex.modData('Learnsets', pokemon.toLowerCase()).learnset[move[0].toLowerCase()] = [`${move[1]}M`];
		}
	}
